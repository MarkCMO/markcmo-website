// WETYR Studios - Budget BACKGROUND function
// Generates AICP top-sheet budget. Async because line-item output for
// studio/tentpole tier exceeds the sync 26s budget.
// Result persisted to Supabase wetyr_jobs; client polls /script-result.
...
exports.handler = async (event) => {
  ...
  const { jobId, breakdown, schedule = null, context = {} } = body;
  ...
  try {
    ...
    await setJob(jobId, { progress: 'building budget' });
    ...
    await setJob(jobId, { status: 'complete', progress: 'done', budget, context: ctx });
