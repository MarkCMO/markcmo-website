 ...
 *
 * Run locally (Cloudflare Pages project):
 *   wrangler pages secret list --project-name=markcmo   (verify vars set)
 *
 * Easiest invocation: source values from your local .env, then run:
 *   bash -c 'set -a; source .env; set +a; node scripts/seed-cirilo.js'
 *
 * Or PowerShell with explicit values:
 *   $env:MARKCMO_SUPABASE_URL="https://saoomfwycegflxelggxv.supabase.co"
 *   $env:MARKCMO_SUPABASE_SERVICE_KEY="..."
...
if (!SUPABASE_URL || !SERVICE_KEY) {
  ...
  console.error('Try (bash):       set -a; source .env; set +a; node scripts/seed-cirilo.js');
  console.error('Try (PowerShell): $env:MARKCMO_SUPABASE_SERVICE_KEY="..."; node scripts/seed-cirilo.js');
  process.exit(1);
}
...
const ENGAGEMENT_DATA = {
  ...
  metadata: {
    ...
    sign_url: '/forms/cirilo-design-build-growth.html',
    blended_aov: 225000,
