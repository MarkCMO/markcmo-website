#!/usr/bin/env python3
"""
inject-subdirs-table.py
Adds the MarkCMO vs Alternatives comparison table to city-pages/ and
location/ pages that have FAQ sections but no comparison table yet.

Inserts immediately before the <div class="sp-faq"> wrapper.

Run from repo root:
  python scripts/inject-subdirs-table.py [--dry-run]
"""
...
def walk():
    cu = lu = 0
    for subdir, counter in [('city-pages', 'c'), ('location', 'l')]:
        for entry in os.scandir(os.path.join(ROOT, subdir)):
            if entry.is_file() and entry.name.endswith('.html'):
                if inject_table(entry.path):
                    if counter == 'c': cu += 1
                    else: lu += 1
                    if DRY_RUN: print(f'[DRY] {subdir}/{entry.name}')
    return cu, lu

if __name__ == '__main__':
    print(f'Running in {"DRY-RUN" if DRY_RUN else "LIVE"} mode...')
    cu, lu = walk()
    print(f'city-pages: {cu}  location: {lu}')
