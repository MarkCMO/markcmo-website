...
(function () {
  ...
  // ── Render everything ─────────────────────────────────────────
  function render(){ renderKpis(); renderKanban('mini-kanban',true); renderKanban('full-kanban',false); renderDashLeads(); renderLeads(); renderClients(); renderAnalytics(); renderReferrals(); renderPayments(); renderEmail(); renderVendorsAdmin(); renderFinancials(); renderProjectsEditor(); renderNeedsAttention(); }
  ...
  // ── Projects & Timeline editor (admin writes the Owner Suite data points) ──
  var PJ_STAGES=['consultation','design','proposal','contract','excavation','rebar_bonding','plumbing_electrical','inspections','shotcrete','tile_coping','equipment','decking','interior_finish','fill_startup'];
  function pjHdr(){ return {'Content-Type':'application/json','x-cdb-admin':sessionStorage.getItem('cdb_admin')||''}; }
  function pjLabel(s){ return String(s||'').replace(/_/g,' ').replace(/\b\w/g,function(m){return m.toUpperCase();}); }
  function pjEsc(s){ return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
  function pjVal(id){ var e=document.getElementById(id); return e?e.value:''; }
  function pjField(label,id,v,type){ return '<div style="margin-bottom:10px;"><label style="display:block;font-family:\'DM Mono\',monospace;font-size:0.6rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--muted);margin-bottom:4px;">'+label+'</label><input id="'+id+'" type="'+(type||'text')+'" value="'+pjEsc(v)+'" style="width:100%;border:1.5px solid var(--border);border-radius:8px;padding:8px 10px;font-size:0.92rem;"></div>'; }
  function renderProjectsEditor(){
    var el=document.getElementById('projects-editor'); if(!el) return;
    var ps=(typeof DATA!=='undefined'&&DATA.projects)||[];
    var head='<div style="padding:0 0 14px;"><button class="kcount" style="cursor:pointer;border:none;background:var(--green);font-size:0.8rem;" onclick="newProject()">+ New client + project</button></div>';
    if(!ps.length){ el.innerHTML=head+'<p class="empty">No projects yet. Create one above, or run the demo seed.</p>'; return; }
    el.innerHTML=head+'<table><thead><tr><th>Project</th><th>Stage</th><th>Contract</th><th></th></tr></thead><tbody>'+ps.map(function(p){
      return '<tr><td><strong>'+pjEsc(p.name)+'</strong></td><td>'+pjLabel(p.stage)+'</td><td class="mono">'+money(+p.value||0)+'</td><td><button class="kcount" style="cursor:pointer;border:none;background:var(--navy);color:#fff;" onclick="openProj(\''+p.id+'\')">Manage</button></td></tr>';
    }).join('')+'</tbody></table>';
  }
  window.renderProjectsBack=function(){ renderProjectsEditor(); };
  window.newProject=function(){
    mcModalForm({title:'New client + project',sub:'Creates the client (Owner Suite login) and their project.',submitLabel:'Create',fields:[
      {name:'client_name',label:'Client name',placeholder:'The Harrington Residence'},
      {name:'client_email',label:'Client email (Owner Suite login)',type:'email',placeholder:'owner@email.com'},
      {name:'neighborhood',label:'Neighborhood',placeholder:'Myers Park, Charlotte'},
      {name:'project_name',label:'Project name',placeholder:'Myers Park Vanishing Edge'},
      {name:'pool_type',label:'Pool type',placeholder:'Custom Gunite'},
      {name:'contract_value',label:'Contract value (USD)',type:'number',placeholder:'285000'}
    ]}).then(function(v){ if(!v) return;
      if(!v.client_name||!v.project_name){ mcToast('Client name and project name are required.','error'); return; }
      fetch('/api/admin-project',{method:'POST',headers:pjHdr(),body:JSON.stringify({op:'create_client_project',client_name:v.client_name,client_email:v.client_email,neighborhood:v.neighborhood,project_name:v.project_name,pool_type:v.pool_type,contract_value:v.contract_value})})
        .then(function(r){return r.json();}).then(function(j){
          if(j&&j.ok){ mcToast('Created. Owner Suite login code: '+(j.portal_code||'(set)'),'success'); loadData(); }
          else mcToast('Could not create: '+((j&&j.error)||'error'),'error');
        }).catch(function(){ mcToast('Could not reach the server.','error'); });
    });
  };
  window.openProj=function(id){
    var el=document.getElementById('projects-editor'); if(!el) return;
    el.innerHTML='<p class="empty">Loading project...</p>';
    fetch('/api/admin-project',{method:'POST',headers:pjHdr(),body:JSON.stringify({op:'get',project_id:id})})
      .then(function(r){return r.json();}).then(function(j){
        if(!j||!j.ok){ el.innerHTML='<p class="empty">Could not load project. '+pjEsc((j&&j.error)||'')+'</p>'; return; }
        renderProjEditor(el,j.project,j.events||[],j.client);
      }).catch(function(){ el.innerHTML='<p class="empty">Could not reach the server.</p>'; });
  };
  function renderProjEditor(el,p,events,client){
    var stageOpts=PJ_STAGES.map(function(s){return '<option value="'+s+'"'+(s===p.stage?' selected':'')+'>'+pjLabel(s)+'</option>';}).join('');
    var h='';
    h+='<div style="padding:0 0 14px;"><button class="kcount" style="cursor:pointer;border:1px solid var(--line);background:#fff;" onclick="renderProjectsBack()">&#8592; All projects</button></div>';
    h+='<div style="border:1px solid var(--line);border-radius:10px;padding:16px 18px;margin-bottom:16px;"><h3 style="margin-bottom:10px;">Project details <span style="color:var(--muted);font-weight:400;font-size:0.8rem;">- shows on the Owner Suite</span></h3>';
    h+=pjField('Project name','pj-name',p.name||'');
    h+=pjField('Pool type','pj-pool',p.pool_type||p.project_type||'');
    h+=pjField('Contract value (USD)','pj-cv',p.contract_value||'','number');
    h+='<div style="margin-bottom:10px;"><label style="display:block;font-family:\'DM Mono\',monospace;font-size:0.6rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--muted);margin-bottom:4px;">Stage</label><select id="pj-stage" style="width:100%;border:1.5px solid var(--border);border-radius:8px;padding:8px 10px;">'+stageOpts+'</select></div>';
    h+=pjField('Start date','pj-start',(p.start_date||'').slice(0,10),'date');
    h+=pjField('Target complete','pj-target',(p.target_complete||p.target_complete_date||'').slice(0,10),'date');
    h+='<button class="kcount" style="cursor:pointer;border:none;background:var(--green);" onclick="saveProj(\''+p.id+'\')">Save project</button></div>';
    if(client){
      h+='<div style="border:1px solid var(--line);border-radius:10px;padding:16px 18px;margin-bottom:16px;"><h3 style="margin-bottom:10px;">Client <span style="color:var(--muted);font-weight:400;font-size:0.8rem;">- Owner Suite login + contact</span></h3>';
      h+=pjField('Name','cl-name',client.name||'');
      h+=pjField('Email (login)','cl-email',client.email||'');
      h+=pjField('Neighborhood','cl-hood',client.neighborhood||client.address||'');
      h+=pjField('Portal access code','cl-code',client.portal_code||'');
      h+='<button class="kcount" style="cursor:pointer;border:none;background:var(--green);" onclick="saveClient(\''+client.id+'\')">Save client</button></div>';
    }
    h+='<div style="border:1px solid var(--line);border-radius:10px;padding:16px 18px;margin-bottom:16px;"><h3 style="margin-bottom:10px;">Progress photo <span style="color:var(--muted);font-weight:400;font-size:0.8rem;">- adds to the Owner Suite gallery</span></h3>';
    h+=pjField('Image URL (https://...)','ph-url','');
    h+=pjField('Caption','ph-cap','');
    h+='<button class="kcount" style="cursor:pointer;border:none;background:var(--navy);color:#fff;" onclick="addPhoto(\''+p.id+'\')">Add photo</button></div>';
    h+='<div style="border:1px solid var(--line);border-radius:10px;padding:16px 18px;"><h3 style="margin-bottom:10px;">Timeline <span style="color:var(--muted);font-weight:400;font-size:0.8rem;">- the updates the homeowner sees</span></h3><div id="pj-events">';
    h+=(events.length?events.map(function(e){
      var note=(e.detail&&e.detail.note)||''; var when=(e.created_at||'').slice(0,10);
      return '<div style="display:flex;gap:10px;align-items:center;justify-content:space-between;border-bottom:1px solid var(--line);padding:8px 0;"><div><strong>'+pjEsc(pjLabel(e.event))+'</strong>'+(note?(' <span style="color:var(--muted);">- '+pjEsc(note)+'</span>'):'')+'<br><span class="mono" style="font-size:0.7rem;color:var(--muted);">'+when+'</span></div><button class="kcount" style="cursor:pointer;border:none;background:#c0392b;color:#fff;" onclick="delEvent(\''+e.id+'\',\''+p.id+'\')">Delete</button></div>';
    }).join(''):'<p class="empty" style="padding:8px 0;">No timeline entries yet.</p>');
    h+='</div><div style="margin-top:16px;border-top:2px solid var(--line);padding-top:14px;"><strong style="font-size:0.85rem;">Add a timeline update</strong>';
    h+=pjField('Title (e.g. Excavation complete)','ev-title','');
    h+=pjField('Note (optional)','ev-note','');
    h+=pjField('Date','ev-date',new Date().toISOString().slice(0,10),'date');
    h+='<button class="kcount" style="cursor:pointer;border:none;background:var(--navy);color:#fff;" onclick="addEvent(\''+p.id+'\')">Add to timeline</button></div></div>';
    el.innerHTML=h;
  }
  window.saveProj=function(id){
    var f={name:pjVal('pj-name'),pool_type:pjVal('pj-pool'),contract_value:pjVal('pj-cv'),stage:pjVal('pj-stage'),start_date:pjVal('pj-start'),target_complete:pjVal('pj-target')};
    fetch('/api/admin-project',{method:'POST',headers:pjHdr(),body:JSON.stringify({op:'update_project',project_id:id,fields:f})})
      .then(function(r){return r.json();}).then(function(j){ mcToast(j&&j.ok?'Project saved. The Owner Suite is updated.':'Could not save project.',j&&j.ok?'success':'error'); if(j&&j.ok) loadData(); }).catch(function(){ mcToast('Could not reach the server.','error'); });
  };
  window.saveClient=function(id){
    var f={name:pjVal('cl-name'),email:pjVal('cl-email'),neighborhood:pjVal('cl-hood'),portal_code:pjVal('cl-code')};
    fetch('/api/admin-project',{method:'POST',headers:pjHdr(),body:JSON.stringify({op:'update_client',client_id:id,fields:f})})
      .then(function(r){return r.json();}).then(function(j){ mcToast(j&&j.ok?'Client saved.':'Could not save client.',j&&j.ok?'success':'error'); }).catch(function(){ mcToast('Could not reach the server.','error'); });
  };
  window.addPhoto=function(pid){
    var url=pjVal('ph-url'); if(!/^https?:\/\//i.test(url)){ mcToast('Enter a valid image URL (https://...).','error'); return; }
    fetch('/api/admin-project',{method:'POST',headers:pjHdr(),body:JSON.stringify({op:'add_photo',project_id:pid,url:url,caption:pjVal('ph-cap')})})
      .then(function(r){return r.json();}).then(function(j){ if(j&&j.ok){ mcToast('Photo added to the Owner Suite gallery.','success'); openProj(pid); } else mcToast('Could not add photo: '+((j&&j.error)||''),'error'); }).catch(function(){ mcToast('Could not reach the server.','error'); });
  };
  window.addEvent=function(pid){
    var title=pjVal('ev-title'); if(!title){ mcToast('Enter a title for the update.','error'); return; }
    fetch('/api/admin-project',{method:'POST',headers:pjHdr(),body:JSON.stringify({op:'add_event',project_id:pid,event:title,note:pjVal('ev-note'),at:pjVal('ev-date')})})
      .then(function(r){return r.json();}).then(function(j){ if(j&&j.ok){ mcToast('Timeline updated.','success'); openProj(pid); } else mcToast('Could not add update.','error'); }).catch(function(){ mcToast('Could not reach the server.','error'); });
  };
  window.delEvent=function(eid,pid){
    mcConfirm({title:'Delete update',message:'Remove this timeline entry? The homeowner will no longer see it.',confirmLabel:'Delete'}).then(function(ok){ if(!ok) return;
      fetch('/api/admin-project',{method:'POST',headers:pjHdr(),body:JSON.stringify({op:'delete_event',event_id:eid})})
        .then(function(r){return r.json();}).then(function(j){ if(j&&j.ok){ mcToast('Update removed.','info'); openProj(pid); } else mcToast('Could not remove update.','error'); }).catch(function(){ mcToast('Could not reach the server.','error'); });
    });
  };
  ...
  function renderVendorsAdmin(){
    ...
    var vt=document.getElementById('vendors-table'); if(vt) vt.innerHTML='<table><thead><tr><th>Vendor</th><th>Trade</th><th>Email</th><th>Status</th><th></th></tr></thead><tbody>'+(V.vendors||[]).map(function(v){
      ...
      var act=v.status==='pending'?('<button class="kcount" style="cursor:pointer;border:none;background:var(--green);" onclick="approveVendor(\''+v.id+'\')">Approve</button> <button class="kcount" style="cursor:pointer;border:none;" onclick="declineVendor(\''+v.id+'\')">Decline</button>'):(v.status==='active'?('<button class="kcount" style="cursor:pointer;border:none;background:var(--navy);color:#fff;" onclick="assignVendor(\''+v.id+'\')">Assign</button>'):'');
  ...
  window.assignVendor=function(vid){
    var v=(VDATA.vendors||[]).find(function(x){return x.id===vid;});
    var projs=(typeof DATA!=='undefined'&&DATA.projects)||[];
    if(!projs.length){ mcToast('Create a project first, then assign a vendor.','error'); return; }
    mcModalForm({title:'Assign '+((v&&v.name)||'vendor')+' to a project',submitLabel:'Assign',fields:[
      {name:'project_id',label:'Project',type:'select',options:projs.map(function(p){return {value:p.id,label:p.name};}),required:true},
      {name:'stage',label:'Stage',placeholder:'shotcrete'},
      {name:'scope',label:'Scope',type:'textarea',placeholder:'Gunite shell incl. infinity-edge wall'},
      {name:'amount',label:'Amount (USD)',type:'number',placeholder:'52000'}
    ]}).then(function(val){ if(!val||!val.project_id) return; vendorPost({op:'assign',vendor_id:vid,project_id:val.project_id,stage:val.stage,scope:val.scope,amount:val.amount?+val.amount:null}); mcToast('Vendor assigned.','success'); });
  };
